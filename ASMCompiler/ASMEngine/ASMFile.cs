namespace ASMEngine
{
    public class ASMFile
    {
        ...
        private int DBDWParse(string s)
        {
            s = s.ToUpper();
            if (s == "DB")
                return 1;
        ...
        private void compile()
        {
                            ...
                            case "DW":
                                {
                                    ...
                                    parsed.Add(new AddressOperator(parts[1], DBDWParse(parts[0])));
                                    break;
                                }
                            default:
                                {
                                    ...
                                    {
                                        table.Add(parts[0]);
                                        parsed.Add(new AddressOperator(parts[2], DBDWParse(parts[1])));
                                        table.Set(parts[0], curAddress);
                                    }
                                    else
                                    {
            ...
            foreach (Operator y in parsed)
            {
                if (y.hasAddress)
                {
                    ...
                    if (y is OperatorsLib.ControlCMDs.JMPOperator || y is OperatorsLib.ConditionCMDs.JxxOperator)
                    {
                        y.CodeLine += Convertors.ExpandTo(table.GetOffset(name, offset - y.address - y.length, 2), 2);
                    }
                    else
                    {
                        if (y is OperatorsLib.ConditionCMDs.LOOPOperator)
